...
export default function VercelSnakeGame() {
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-3 select-none font-sans">
      {/* HEADER HUD / BRANDING VERCEL */}
      <header className="w-full max-w-[800px] mb-3 flex items-center justify-between bg-zinc-950/80 backdrop-blur border border-zinc-800 rounded-xl p-3 shadow-2xl">
        <div className="flex items-center space-x-3">
          <div className="w-7 h-7 bg-white clip-triangle flex items-center justify-center shadow-glow">
            <svg width="18" height="18" viewBox="0 0 76 65" fill="none">
              <path d="M37.5274 0L75.0548 65H0L37.5274 0Z" fill="black" />
            </svg>
          </div>
          <div>
            <h1 className="font-bold text-lg tracking-wider bg-gradient-to-r from-white via-zinc-200 to-cyan-400 bg-clip-text text-transparent">
              VERCEL SNAKE
            </h1>
      ...
      <div className="w-full max-w-[800px] mb-2 grid grid-cols-4 gap-2 text-center text-xs font-mono">
        <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-lg p-2">
          <div className="text-zinc-500 uppercase">PONTOS</div>
          <div className="text-cyan-400 font-bold text-base">{score}</div>
        </div>
        <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-lg p-2">
          <div className="text-zinc-500 uppercase">RECORDE</div>
          <div className="text-amber-400 font-bold text-base">{highScore}</div>
        </div>
        <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-lg p-2">
          <div className="text-zinc-500 uppercase">NÍVEL</div>
          <div className="text-purple-400 font-bold text-base">{level}</div>
        </div>
        <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-lg p-2">
          <div className="text-zinc-500 uppercase">COMBO</div>
          <div className="text-pink-400 font-bold text-base">{combo}x</div>
        </div>
      </div>
      ...
      <div className="relative w-full max-w-[800px] aspect-[4/3] bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden shadow-2xl">
        <canvas
          ref={canvasRef}
          width={800}
          height={600}
        ...
        {status === 'menu' && (
          <div className="absolute inset-0 bg-black/90 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center z-20">
            ...
            <p className="text-zinc-400 text-xs max-w-md mb-4 font-mono">
              O clássico da Nokia com físicas Canvas, 40+ emojis, ranking global Turso SQLite e personalização de avatar.
            </p>
      ...
      <div className="w-full max-w-[800px] mt-4 flex flex-col items-center justify-center md:hidden">
        <div className="grid grid-cols-3 gap-2 w-44">
          <div />
          <button
            onClick={() => changeDirection({ x: 0, y: -1 })}
            className="h-12 rounded-lg bg-zinc-900 active:bg-cyan-500 active:text-black border border-zinc-700 text-xl font-bold flex items-center justify-center shadow"
          >
            ▲
          </button>
          <div />
        ...
        <p className="text-[10px] text-zinc-500 font-mono mt-2">
          Use os botões virtuais ou deslize o dedo na tela
        </p>
      </div>

      {/* FOOTER & TECLAS ATALHO */}
      <footer className="mt-4 text-center text-zinc-500 text-xs font-mono">
