    ...
    const draw = (interval: number, time: number) => {
      ...
      snake.forEach((seg, idx) => {
        ...
        if (isHead) {
          ...
          // Indicador de intenção: quando há curva enfileirada, um nub apontando
          // p/ a direção que vai executar (ensina a bufferizar; resposta imediata)
          if (queued) {
